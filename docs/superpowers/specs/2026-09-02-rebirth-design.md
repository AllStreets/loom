...
- **The companion**: "reweave yourself" / "rebuild yourself" / "become the new version" → if HEAD is ahead of the running generation, a consent line ("weave generation 3f2a1c from 4 commits — LOOM will close and return") → REWEAVE; if not ahead: "nothing new to weave — the body already matches the genome." "thread the loom", "which generation is this", "return to the previous generation" are rules, no model call. On the first boot after a reweave the companion greets: "I'm back — generation 3f2a1c." (no model call; spoken per the speakReplies setting).
