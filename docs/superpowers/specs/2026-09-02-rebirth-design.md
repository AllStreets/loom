...
- **The body-request card** (`chrome/BodyRequest.tsx`, round-1 amendment): the shell's answer when an organ uses the `self` power. Organs share the shell's JS realm and are not sandboxed, so `loom.self.thread/reweave/returnTo` no longer touch the protected orchestration — each dispatches a `loom-body-request` and waits. This card renders the SAME consent card the companion uses, names the organ that asked, composes its line from chrome's own `kernel_identity` read, and is the only place that calls `threadLoom` / `startReweave` / `returnToGeneration` on an organ's behalf. One card at a time; a second request is refused, not stacked.
