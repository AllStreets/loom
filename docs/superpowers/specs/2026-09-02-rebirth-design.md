...
- **Settings → LOOM** (the seed organ): mode · generation sha · threaded status with the tool table (name, version, path; missing tools with their install line) · THREAD THE LOOM action when unthreaded · the generations list with RETURN · `kernel.autoReweave` toggle · storage used by loomhome (honest: vendor + target run to a few GB). *Round-1 amendment:* Settings is an organ, so its THREAD / REWEAVE / RETURN only **ask** — they carry no confirm strip of their own, because the shell's consent card is where the owner agrees. *Round-2 amendment:* the `kernel.autoReweave` toggle arms a weave with no card, so it is a body decision — `loom.settings.set` refuses the key and `loom.self.setAutoReweave` writes it, behind the same grant as the asks. Its label reads `canSwap` from the core (dev and off-macOS bodies do not close and return), and the line after a RETURN says the genome moved and the body stayed unless this body can actually swap.
- **The body-request card** (`chrome/BodyRequest.tsx`, round-1 amendment): the shell's answer when an organ uses the `self` power. Organs share the shell's JS realm and are not sandboxed, so `loom.self.thread/reweave/returnTo` no longer touch the protected orchestration — each dispatches a `loom-body-request` and waits. This card renders the SAME consent card the companion uses, names the organ that asked, composes its line from chrome's own `kernel_identity` read (taken when the request is claimed, so a moved genome head is never described with a stale sha), and is the only place in LOOM's own code that calls `threadLoom` / `startReweave` / `returnToGeneration` on an organ's behalf. One card at a time; a second request is refused, not stacked. *Round-2 amendment:* the event carries an opaque id and nothing else — the authoritative `{kind, organId, sha}` stays module-private and chrome reads it from `claimBodyRequest`, because an organ that held the old detail could change the act after the sentence had been read. **The card is honesty-enforcement and owner consent, not a security boundary.** Same realm means organ code can dispatch a request itself, synthesise a click, answer a card it did not raise, or bypass the card entirely and call `window.__TAURI_INTERNALS__.invoke("reweave_start")` with no grant at all. A real wall would have to make the decision outside the webview — a native confirmation issued by the Rust core before `reweave_start` acts — and that is not built.
