...
describe("requestImpliesPowers", () => {
  it("matches every keyword class the powers cover", () => {
    const powered = [
      "alert me when the kettle boils",          // notify
      "speak the time on the hour",              // voice
      "remind me to stretch",                    // pulse
      "check the folder every ten minutes",      // pulse
  ...
  it("never fires on plain widget requests", () => {
    const plain = [
      ...
      "show the price of ethereum",              // market is retired — no power
      "what's in the news right now",            // watch is retired — no power
    ];
...
describe("POWERS_CONTRACT", () => {
  it("names all four powers and the manifest declaration requirement", () => {
  ...
  it("states the budgets and pulse limits", () => {
    expect(POWERS_CONTRACT).not.toContain("market");
  ...
  it("embeds the worked stand-up few-shot verbatim", () => {
...
describe("organSystemPrompt powers injection", () => {
  const powered = "remind me to stand up every hour";
  ...
  it("injects the POWERS block for powered requests in every builder kind", () => {
    for (const kind of ["manifest", "code", "tests", "repair"] as const) {
      ...
      expect(prompt, kind).toContain("POWERS — four gated capabilities");
  ...
  it("omits the block for plain requests and when no request is given", () => {
    expect(organSystemPrompt("code", { request: "water tracker" })).not.toContain("POWERS — four gated capabilities");
    expect(organSystemPrompt("code")).not.toContain("POWERS — four gated capabilities");
  });
  ...
  it("sits between the contract and the experience block", () => {
    ...
    const contractPos = prompt.indexOf("An ORGAN is a small self-contained tool");
    const powersPos = prompt.indexOf("POWERS — four gated capabilities");
    const expPos = prompt.indexOf("EXPERIENCE — PAST SUCCESSFUL BUILDS");
    const taskPos = prompt.indexOf("Now output organ.js only");
    expect(contractPos).toBeGreaterThan(-1);
...
describe("POWERS few-shot fixture (offline gate proof)", () => {
  it("manifest passes manifestGuard with the declared powers", () => {
    const r = manifestGuard(POWERS_FEWSHOT.manifest, "stand-up-reminder");
    expect(r.ok, r.ok ? "" : (r as { ok: false; error: string }).error).toBe(true);
    if (r.ok) expect(r.manifest.powers).toEqual(["notify", "pulse", "voice"]);
  });
  ...
  it("gate() carries the declared powers through to its manifest", async () => {
    ...
    const r = await gate({ manifest: POWERS_FEWSHOT.manifest, code: POWERS_FEWSHOT.code, tests: POWERS_FEWSHOT.tests }, "stand-up-reminder");
    expect(r.ok).toBe(true);
    expect(r.manifest?.powers).toEqual(["notify", "pulse", "voice"]);
    spy.mockRestore();
  });
  ...
  it("the organ renders and its tests pass against the harness power mocks", async () => {
    ...
    expect(organ.id).toBe("stand-up-reminder");
  ...
  it("a second nudge on the same day counts two — the per-day baseline the few-shot teaches", async () => {
    const organ = new Function(POWERS_FEWSHOT.code.replace(/^export\s+default\s+/m, "return "))() as FixtureOrgan;
    const el = document.createElement("div");
    const loom = harnessLoom();
    await organ.render(el, loom);      // the mock pulse fires once on registration
    await organ.render(el, loom);      // and once more — same storage, same day
    await new Promise((r) => setTimeout(r, 0));
    expect(loom.notify.sent).toHaveLength(2);
    expect(loom.storage.get("nudges", null)).toMatchObject({ n: 2 });
  });
...
describe("selfEditSystemPrompt", () => {
  ...
  it("does NOT drag in the ORGAN_CONTRACT or POWERS block (lean, distinct prompt)", () => {
    ...
    expect(p).not.toContain("POWERS — four gated capabilities");
...
describe("RUST self-edit contract — absent from organ builds", () => {
  it("no organ build kind leaks the RUST self-edit contract", () => {
    for (const kind of ["manifest", "code", "tests", "repair", "edit"] as const) {
      const prompt = organSystemPrompt(kind, { request: "remind me to stand up every hour" });
